...
class Perceptron(object):
    ...
    def calcularErro(self,valorReal,vetorYatual):
        if(len(vetorYatual)==len(self.yDesejado)): #significa q o Y desejado foi montado corretamente
            ...
            resultado = resultado/len(self.sinapses)
    ...
    ###########
    def carregarSinapses(self,numeroSinapses=10, resolucao=16):
        bd = SinapseDB('bd/rna')
        self.sinapses=[]
        for i in range(numeroSinapses):
            temp = bd.carregarSinapse((i+1))
    ...
    def gerarSinapses(self, numeroSinapses=10, resolucao=16):
        bd = SinapseDB('bd/rna')
        for i in range(numeroSinapses):
            temp = []
    ...
    def reconhecimento(self, vetorNovo):
        ...
        for i in range(len(self.sinapses)):
            erroCalculado, vetorErro = self.calcularErro((i+1), yVector)
